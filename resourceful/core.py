...
import resourceful.checksum
import resourceful.registry

DEFAULT_SIGNATURE = 'resourceful'

VERSION_PREFIX = ':version:'

BUNDLE_PREFIX = ':bundle:'

NEEDED = 'resourceful.needed'
...
def set_resource_file_existence_checking(v):
    ...
    it at this value when using Resourceful. An
    ...
    False temporarily; for the Resourceful tests this is done. Inside
...
class Resource(Renderable, Dependable):
    ...
    def __init__(self, library, relpath,
        ...
        self.compiler = resourceful.registry.CompilerRegistry.instance()[
            compiler]
        self.source = source
        ...
        if minifier is NOTHING:
            if mode_parent is None:
                ...
                minifier = None
        self.minifier = resourceful.registry.MinifierRegistry.instance()[
            minifier]
...
class NeededResources(object):
    ...
    :param versioning: If ``True``, Resourceful will automatically include
    ...
    :param versioning_use_md5: If ``True``, Resourceful will use and md5
    ...
    :param recompute_hashes: If ``True`` and versioning is enabled, Resourceful
    ...
    :param bottom: If set to ``True``, Resourceful will include any
    ...
    :param minified: If set to ``True``, Resourceful will include all
      resources in ``minified`` form. If a Resource instance does not
      provide a ``minified`` mode, the "main" (non-named) mode is used.

    :param debug: If set to ``True``, Resourceful will include all
    ...
    :param publisher_signature: The name under which resource libraries
      should be served in the URL. By default this is ``resourceful``, so
      URLs to resources will start with ``/resourceful/``.

    :param bundle: If set to True, Resourceful will attempt to bundle
