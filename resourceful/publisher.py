...
class LibraryPublisher(webob.static.DirectoryApp):
    """Resourceful directory publisher WSGI application.

    This WSGI application serves a directory of static resources to
    the web.

    This WSGI component is used automatically by the
    :py:func:`Resourceful` WSGI framework component, but can also be
    used independently if you need more control.

    :param library: The resourceful library instance.
    """
    ...
    def __call__(self, req):
        ...
        if app is None:
            ...
            elif resourceful.BUNDLE_PREFIX in path:
                # We are handling a bundle request.
                subdir, bundle = req.path_info.split(resourceful.BUNDLE_PREFIX, 1)
...
class Publisher(object):
    """Resourceful publisher WSGI application.

    This WSGI application serves Resourceful :py:class:`Library`
    ...
    def __call__(self, request):
        ...
        if potential_version is not None and \
                potential_version.startswith(resourceful.VERSION_PREFIX):
...
class Delegator(object):
    """Resourceful delegator WSGI framework component.

    This WSGI component recognizes URLs that point to Resourceful
...
def make_publisher(app, global_config,
                   publisher_signature=resourceful.DEFAULT_SIGNATURE):
    publisher = Publisher(resourceful.LibraryRegistry.instance())
