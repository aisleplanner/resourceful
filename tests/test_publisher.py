...
from resourceful import LibraryRegistry, Library, Publisher, Delegator, Resource
from resourceful.publisher import FOREVER
...
def test_delegator(tmpdir):
    ...
    request = webob.Request.blank('/resourceful/foo/test.js')
    response = request.get_response(delegator)
    assert response.body == b'/* a test */'
    assert response.content_type == 'text/javascript'

    # A deeper resourceful.
    request = webob.Request.blank('/foo/bar/resourceful/foo/test.js')
    response = request.get_response(delegator)
    assert response.body == b'/* a test */'
    assert response.content_type == 'text/javascript'
