from resourceful import Library, Resource, NeededResources
from resourceful import compat
from resourceful import set_resource_file_existence_checking as check_files
from resourceful.compiler import Compiler, Minifier
from .test_checksum import _copy_testdata
from zipfile import ZipFile
import resourceful
import resourceful.compiler
import os
import pytest
import subprocess
import sys
...
class MockCompiler(resourceful.compiler.Compiler):
...
class MockMinifier(resourceful.compiler.Minifier):
...
class TestingRegistry(object):
    ...
    def add_compiler(self, compiler):
        return self._register_compiler(
            resourceful.CompilerRegistry, compiler)

    def add_minifier(self, compiler):
        return self._register_compiler(
            resourceful.MinifierRegistry, compiler)
    ...
    def compiler(self, name):
        return resourceful.CompilerRegistry.instance()[name]

    def minifier(self, name):
        return resourceful.MinifierRegistry.instance()[name]
...
def test_logging_when_compiling(tmpdir, compilers, caplog):
    class WhiteSpaceRemover(resourceful.compiler.Compiler):
...
def test_settings_on_resource_override_settings_from_library(compilers):
    ...
    assert isinstance(a.minifier, resourceful.compiler.NullCompiler)
...
def test_compiler_available_and_source_not_present_should_raise(
    ...
    check_files(True)
    lib = Library('lib', str(tmpdir))
    with pytest.raises(resourceful.UnknownResourceError) as exc:
        a = Resource(lib, 'a.js', compiler='mock')
...
def test_minifier_available_and_minified_not_a_string_should_raise(compilers):
    ...
    with pytest.raises(resourceful.ConfigurationError) as exc:
...
def test_cli_compiler_is_not_available_if_command_not_found_on_path():
    class Nonexistent(resourceful.compiler.CommandlineBase):
        command = 'does-not-exist'
    assert not Nonexistent().available


def test_cli_compiler_is_available_if_command_found_on_path():
    class Cat(resourceful.compiler.CommandlineBase):
        command = 'cat'
...
def test_converts_placeholders_to_arguments(tmpdir):
    from resourceful.compiler import SOURCE, TARGET
    ...
    class Echo(resourceful.compiler.CommandlineBase):
        command = 'echo'
...
def test_coffeescript_compiler(tmpdir):
    compiler = resourceful.CompilerRegistry.instance()['coffee']
...
def test_less_compiler(tmpdir):
    compiler = resourceful.CompilerRegistry.instance()['less']
    if not compiler.available:
        pytest.skip('`%s` not found on PATH' % compiler.command)
...
def test_sass_compiler(tmpdir):
    compiler = resourceful.CompilerRegistry.instance()['sass']
    if not compiler.available:
        pytest.skip('`%s` not found on PATH' % compiler.command)
...
def test_package_compiler_is_not_available_if_package_not_importable():
    class Nonexistent(resourceful.compiler.PythonPackageBase):
        package = 'does-not-exist'
    assert not Nonexistent().available


def test_package_compiler_is_available_if_package_is_importable():
    class Example(resourceful.compiler.PythonPackageBase):
        package = 'resourceful'
    assert Example().available


def test_cssmin_minifier(tmpdir):
    compiler = resourceful.MinifierRegistry.instance()['cssmin']
    if not compiler.available:
...
def test_jsmin_minifier(tmpdir):
    compiler = resourceful.MinifierRegistry.instance()['jsmin']
    if not compiler.available:
        pytest.skip('`%s` not found' % compiler.package)
...
def test_closure_minifier(tmpdir):
    compiler = resourceful.MinifierRegistry.instance()['closure']
    if not compiler.available:
        pytest.skip('`%s` not found' % compiler.package)
...
def test_closure_minifier_communicate_exit_status(tmpdir):
    ...
    if not compiler.available:
        pytest.skip('`%s` not found' % compiler.package)
    from resourceful.compiler import CompilerError
...
def libraries(request):
    def cleanup():
        resourceful.LibraryRegistry._instance = None
